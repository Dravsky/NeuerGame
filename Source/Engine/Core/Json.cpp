...
namespace lola
{
	...
	{
		std::string buffer;
		std::filesystem::path filepath(filename);  // Convert string to std::filesystem::path

		// Read the file into the buffer using readFile() from FileIO.h
	...
	bool Json::Read(const rapidjson::Value& value, const std::string& name, int& data, bool required)
	{
		...
		// Get the int data from the Value
